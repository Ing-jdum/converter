...
/**
 * Represents a temperature unit repository implementation.
 * This enum serves as a singleton implementation of the {@link IItemRepository} and {@link ITemperatureUnitRepository} interfaces.
 * It provides methods to retrieve temperature units and their descriptions.
 * 
 * <p>Usage:
 * The repository instance can be accessed through the {@code INSTANCE} field, which represents the singleton instance of the repository.
 * To retrieve all temperature units, use the {@code getAllItems} method, which returns a list of exchangeable temperature units.
 * To get the title of the repository, use the {@code getTitle} method.
 * To retrieve a specific temperature unit by its description, use the {@code getTemperatureUnitByDescription} method.
 * 
...
public enum TemperatureUnitRepositoryImpl implements IItemRepository, ITemperatureUnitRepository {
    ...
    private List<IExchangeableItem> temperatures = Stream.of(ETemperatureUnits.values())
            .map(IExchangeableItem.class::cast)
            .toList();
     ...
     * @return an optional containing the temperature unit if found, or an empty optional if not found
     */
    @Override
